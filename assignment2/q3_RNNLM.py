  ...
  def add_embedding(self):
    ...
    with tf.device('/cpu:0'):
        ...
        with tf.variable_scope("embedding")as embedding_scope:
            L = tf.get_variable(name="lookup_table",shape=[len(self.vocab), self.config.embed_size])
            #initializer=tf.random_uniform_initializer(minval=-1.0,maxval=1.0)

            all_embeddings = tf.nn.embedding_lookup(params=L,ids=self.input_placeholder)
#             print("input before split shape =",inputs)

            #split the input into shape (?, self.config.num_steps, self.config.embed_size)
            embeddings_list = tf.split(all_embeddings,self.config.num_steps,axis=1)
        

#             print("input after split shape =",inputs)

            #squeeze removes all size1 dimensions at dim=1.
            
    inputs = [tf.squeeze(item, axis=1) for item in embeddings_list]

#             print("input after squeeze shape =",inputs)
...
#    outputs = [tf.matmul(tf.nn.dropout(rnn_step, self.dropout_placeholder), U) + b2
  ...
  def add_model(self, inputs):
    ...
    with tf.variable_scope("rnn") as scope:
        ...
        for step, current_input in enumerate(inputs):
            ...
            Z1 =  tf.matmul(state, H) + tf.matmul(current_input,I) + b1
            state = tf.sigmoid(Z1)
